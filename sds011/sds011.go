...
func (resp *response) IsCorrect() error {
	...
	for i := 0; i < 6; i++ {
		...
		log.V(6).Infof("data %d: %v, total: %v", i, resp.Data[i], checkSum)
	}

	if checkSum != resp.CheckSum {
		log.V(6).Infof("bad checksum: '%v' vs '%v'", checkSum, resp.CheckSum)
		return fmt.Errorf("bad checksum: %#v", resp)
	}
	log.V(6).Infof("good checksum: '%v' vs '%v'", checkSum, resp.CheckSum)
	return nil
}
